...
- **pending_ops queue**：所有「想排 / 想取消」的事件先進這個 Redis sorted set，score 編碼 shrink-優先 + 組內 FIFO，worker 用 `ZPOPMIN` 逐筆 O(log n) 取出處理。
- **scheduler state**：序列化在 Redis 的線段樹 + pq + `base_date`，跨次持久保存。
...
```
永遠 202 — 操作只是進佇列，實際排程結果走 WebSocket `schedule.updated` 通知。
...
修法：waiter 在 `_wait_for_idle_run` 結束之後、開始動 state 之前先 `_set_status("running")`，body 結束時在 inner finally 清回 `idle`。從這時起：
...
- inner finally 跟 outer waiter-flag finally 是兩層，先還 status 再清 flag，順序剛好讓「有人剛好在 status 還沒翻 idle 之前 retrigger」這個邊界永遠看到 status=running

例外處理：waiter body 拋例外時，inner finally 把 status 寫成 `idle`（不是 `failed`）。這跟 `run_scheduling_task` 不同 — run_task 的 `failed` 是 endpoint 用來防 trigger 在剛失敗的 worker 上又開一支的 sentinel；waiter 沒占用同一個鎖位，再加上 advance_day 是 cron tick、rebuild 是 user-triggered，下一次能很自然地把 state 寫對，所以單純還 `idle` 就好。Celery 的 traceback 仍然會自己記。

#### `advance_day_task`（每天 00:00 UTC，由 Beat 觸發）

整個 task body 包在 `try / finally` 裡 — 進入時 `_set_waiter_flag()`，離開時 `_clear_waiter_flag()`。等 in-flight run 結束之後，再用一層 inner `try / finally` 把 `schedule:status` claim 成 `running`，body 結束時清回 `idle`。
...
4. `_load_state()` → `advance_day(state)` 算出新 state
5. `_finalize_run(new_state)` — 直接做 compute / apply / save / broadcast，**不靠 `run_scheduling_task`** 幫忙。理由：per-op 設計下 `run_scheduling_task` 在 queue 空時不會 broadcast，所以 advance_day 必須自己 finalize 才能讓前端收到「換天了」的 refresh signal。
6. 只有 `ZCARD pending_ops > 0` 才 `run_scheduling_task.delay()` 把等待期間累積的 ops 消化掉
7. **inner `finally: _set_status("idle")`** — 不論 body 正常或 raise，都把 status 還原
8. **outer `finally: _clear_waiter_flag()`** — 同上，永遠跑

#### `rebuild_schedule_task`（由 `POST /schedule/rebuild` 觸發）

跟 advance_day 一樣套兩層 try / finally：外層持有 waiter flag、內層 claim status。
...
10. **inner `finally: _set_status("idle")`**
11. **outer `finally: _clear_waiter_flag()`**

例外路徑：`run_scheduling_task` 任何步驟丟出 exception，worker 把 status 標 `failed`、寫進 `error` 欄位、re-raise（Celery 也會記錄 traceback）。`advance_day_task` / `rebuild_schedule_task` body 內任何步驟 raise 時，inner finally 會把 status 還原成 `idle`（不寫 `failed` — `failed` 是 run_scheduling_task 專屬的 sentinel，避免一個 advance_day 失敗永久卡住 `/trigger` 的 409 判斷）；outer finally 還會清 waiter flag，最後讓 Celery 自己記錄 traceback。
...
#### `backend/tests/workers/test_scheduling_task.py` — Celery task body（17 個）
...
- **`test_advance_day_clears_status_to_idle_even_on_exception`**（PR-review 補強）

  **在測什麼**：inner finally 的 crash safety — 如果 body 在 status 已 claim `running` 之後 raise（例如 advance_day 演算法 bug、`_finalize_run` 寫 DB 出錯），inner finally 仍要把 status 寫回 `idle`。否則 status 卡在 `running`，後續所有 `/trigger` 永遠 409，唯一解套是 ops 進 Redis 手動改 key（runbook 列在 `scheduling-integration.md` §4.5）。

  **為什麼這條跟 waiter-flag-on-exception 是不同的 invariant**：waiter flag 對應「不要 retrigger 撞到 waiter」；status 對應「不要在 waiter 做事時又開 trigger」。兩條保護寫在不同的 try/finally 層、保護的 key 也不同（`schedule:waiter_pending` vs `schedule:status`），所以要各自有獨立的 crash-safety 測試。重構若把兩層 finally 合併，這條會抓到 status 沒被還原。

  **手法**：跟 waiter-flag-on-exception 同一個樣板 — `advance_day` mock 成 `side_effect=RuntimeError("boom")`、`apply()` 確認 `not result.successful()`、再讀 fake_redis 的 `schedule:status` 驗 `state == "idle"`。
