...
2. 對抓到的 `compounds = [c1, c2, ..., cN]` 做 **halving 二分搜尋找最大可行 prefix**：
   - 試 `[1..N]` → 用 `compute_batch_capacity_delta` 折成 per-day 表 → `is_batch_feasible(state, delta)` 比較 prefix sum 跟 `capacity_tree`。可行就接受。
   - 不可行 → 試 `[1..N//2]`，再 `[1..N//4]`...，到 `[1..1]` 仍不可行就 return `0`。
3. **`k == 0`**（連第一筆 compound 都塞不下）：`_reject_first_compound` ZREM 那筆 + drop secondary index + `_perform_compound_db_action(rejected)` + WS `schedule.compound_failed`。再 continue 外圈 while。
4. **`k > 0`**：`_commit_accepted_batch` 一口氣處理 `compounds[:k]`：
...
5. 外圈 while 再從步驟 1 重來（新 compound 可能在 batch 處理期間進來）。
6. drain 結束才 `materialize_schedule_task.delay()` 一次（**整輪 drain 共用一個 materialize**，不是每 batch 一次）。
7. status 翻回 `idle`，最後檢查 `zcard pending_ops > 0` — 若有（drain 結束跟 status 翻完之間又有 compound 進來），看 `schedule:waiter_pending`：沒設就 self-`.delay()`；有設就 yield 給 waiter。
