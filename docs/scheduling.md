...
7. **success：`_set_status("idle")`**
8. **except：`_set_status("failed", error=str(exc))` 然後 re-raise** — Celery 會把 traceback 寫進 result backend，ops 也能在 `GET /schedule/status` 直接看到 `state="failed"` 跟錯誤訊息
9. **outer `finally: _clear_waiter_flag()`** — 不論 body 正常或 raise 都會跑
