...
#### `GET /schedule/pending_ops` — 排隊中 compound 的順位快照
...
**行為細節**：
- 每個 compound 的所有 ops 共享同一個 `order_id`（schema validator 強制），所以 response 把 `order_id` 提到 top-level 方便前端 group。
- 一筆訂單**可能會出現在 list 中多次**（連續快速 PATCH 在 worker 還沒消化前可以堆兩個 compound）。前端要計算「某訂單的下一個排程動作排第幾」就 filter by `order_id` 取最小 rank。
...
- `ops` 欄只列每個 leaf op 的 `op` 字串（"add" / "remove" / "pin" / "unpin"），讓前端能標「刪除中⋯」/「pin 中⋯」而不用拿完整 op payload。
