...
4. `_process_one(state, op)` 套用單筆 op：`add_order` / `remove_order`，`add` 失敗就 `websocket.notify_user(requested_by, ...)`
...
整個 task body 包在 `try / finally` 裡 — 進入時 `_set_waiter_flag()`，離開時 `_clear_waiter_flag()`。

1. **`_set_waiter_flag()`**（rabbit-flag race fix，見下面 "Waiter-flag race fix" 段）
2. `_wait_for_idle_run(...)` 輪詢 `schedule:status` 等 `running` 結束（最多 5 分鐘，超時就警告繼續）
3. `_load_state()` → `advance_day(state)` 算出新 state
4. `_finalize_run(new_state)` — 直接做 compute / apply / save / broadcast，**不靠 `run_scheduling_task`** 幫忙。理由：per-op 設計下 `run_scheduling_task` 在 queue 空時不會 broadcast，所以 advance_day 必須自己 finalize 才能讓前端收到「換天了」的 refresh signal。
5. 只有 `ZCARD pending_ops > 0` 才 `run_scheduling_task.delay()` 把等待期間累積的 ops 消化掉
6. **`finally: _clear_waiter_flag()`** — 不論 body 正常結束還是 raise，都會跑

#### `rebuild_schedule_task`（由 `POST /schedule/rebuild` 觸發）

跟 advance_day 一樣套 try / finally 持有 waiter flag。

1. **`_set_waiter_flag()`**
2. `_wait_for_idle_run(...)` — 跟 `advance_day_task` 共用同一個等待 helper
3. 從 Redis 讀現有 `schedule:state.base_date`（沒有就用今天）
4. `order_service.list_for_scheduler(db)` 拿出 `(orders, creators)`
5. `rebuild_state(orders, base_date)` 得到 `(new_state, skipped)`
6. `_finalize_run(new_state)` — 同樣自己 finalize（理由同 advance_day）
7. 對每筆 `skipped` 查 `creators` map，`websocket.notify_user(creator, "schedule.rebuild_skipped")`
8. 只有 `ZCARD pending_ops > 0` 才 `run_scheduling_task.delay()` 消化等待期間累積的 ops
9. **`finally: _clear_waiter_flag()`**

例外路徑：`run_scheduling_task` 任何步驟丟出 exception，worker 把 status 標 `failed`、寫進 `error` 欄位、re-raise（Celery 也會記錄 traceback）。`advance_day_task` / `rebuild_schedule_task` 的 exception 不寫 `failed` status（因為它們自己沒佔用 status 為 running），就單純讓 Celery 記錄。
...
| `apply_schedule(db, scheduled: list[ScheduledResult]) -> int` | 編排 5 個步驟（清空 → 聚合 earliest/latest → 逐筆 set → 發 audit log → commit）；回傳被排程的訂單數 | `workers.scheduling.run_scheduling_task` |
...
4. 對每筆訂單發 `emit_audit_log(action="order.scheduled", actor_id=None, ...)`，符合 `docs/DEVELOPMENT_GUIDELINES.md §6` 對 user-visible mutation 的稽核要求；`actor_id=None` 標示系統觸發
5. `db.commit()`
...
#### `backend/tests/workers/test_scheduling_task.py` — Celery task body（14 個）
...
#### `backend/tests/api/test_websocket.py` — WS endpoint + manager（11 個）
