...
| **Compound 通過 fast-path saga**（Phase 4：in-memory state 已接受、DB 寫入排到 materializer 上） | `notify_user` | `{"type": "schedule.compound_accepted", "compound_id": "..."}` |
...
- 三支 state-writing task（run / advance_day / rebuild）進 body 前 `SET NX EX 300 task_id`
- finally 用 Lua CAS-delete（只刪 value == 自己 task_id 的 lock）防止「TTL 過期 → 別人接手 → 我們的 finally 誤刪別人 lock → race window 重開」這個二次 race
- `run_scheduling_task` 拿不到就 return（holder 在自己 retrigger 時會接手）；`advance_day_task` / `rebuild_schedule_task` polling 等 lock，超時 → status=failed（明示卡住、ops 才能介入）
...
**Invariant break 時的處理：`_apply_remove_to_trees` 殘餘 → raise，trigger saga rollback**
...
`_process_compound` 接住這個 `RuntimeError`、走 saga rollback（state 回 snapshot、通知 `compound_failed`、compound 失敗結束）—— 比讓整個 task fail-and-crash 友善：user 可以重試（如果是 transient）、worker 繼續處理下一個 compound（如果是該 compound 特有）。
...
#### `run_scheduling_task` — **per-compound 模式**（Phase 2/4）

每次 task 呼叫處理**一個 compound**（含 N 筆 leaf ops，沒上限），整個 compound 期間 `schedule:status` 維持 `running`，跑完才翻回 `idle`，看還有沒有 pending compound 才再 `.delay()` 自己。
...
3. **任何一筆失敗**：`SchedulerState.from_json(snapshot)` 把 state 還原回 pre-compound → WS `schedule.compound_failed` → return False（**不** save_state、不 dispatch materializer）。
4. **全部成功（Phase 4 fast-path 走法）**：
   - `_save_state(state)` 把更新後的 trees + pq + pinned dict 序列化回 Redis（O(n) 但只是小型 int array + dict，很快）。
   - `websocket.notify_user(requested_by, "schedule.compound_accepted")` 立刻通知 producer「你的 compound 過了」。
   - `enqueue_notify_user(requested_by)` SADD 進 `schedule:materialize_notify_pending`，給後面的 materializer 拿來「DB 寫完之後通知 user 重新 fetch」。
   - `materialize_schedule_task.delay()` 派工讓 deferred 工人去跑 `compute_schedule + apply_schedule` + per-user notify。

`advance_day` / `rebuild` 不能在 compound 中間插隊，因為 status 在整段期間都是 `running`。

**Phase 4 fast / slow 拆分動機**：原本 `run_scheduling_task` 一次處理完一個 compound 就會 `compute_schedule + apply_schedule + broadcast`。`compute_schedule` 跟 `apply_schedule` 各 O(n)（其中 apply_schedule 是 N 筆 DB UPDATE 來回），producer 要等到全部 N 筆 DB 寫完才知道自己的 compound 被接受。user spec 要求 accept/reject 回饋落在 O(log n)·N（= 純 in-memory state 操作）的時間裡，所以把 DB write 整段挪去 deferred task。詳見下面的 `materialize_schedule_task`。
...
| 事件 | 何時發 | 誰收到 |
|---|---|---|
| `schedule.compound_accepted` | fast path saga 全綠之後 | compound 的 `requested_by` |
| `schedule.compound_failed` | fast path saga rollback | compound 的 `requested_by` |
...
- `advance_day_task` / `rebuild_schedule_task` polling 等 lock（`_acquire_state_lock_blocking`），最多等 `SCHEDULER_RUN_WAIT_TIMEOUT_SECONDS`；超時 → status=`failed`（系統真的卡住、與其默默等不如丟出來讓 ops 看到）
