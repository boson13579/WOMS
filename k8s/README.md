...
After a fresh deploy, `25-seed-admin-job.yaml` creates a root user named
`admin` so reviewers can log in without registering. The password is
**generated by Terraform** (in `infra/admin.tf`) and lives only in:

1. `terraform.tfstate` (local, gitignored)
2. The `woms-secrets` K8s Secret (key `ADMIN_PASSWORD`)
...
The seed Job reads the password from the Secret at runtime — the password
is never written to any file under version control. Share with reviewers
out-of-band (Slack, email, etc.).

The Job is "ensure"-style: re-running it updates the existing `admin`
row's email / password / role to match the current Secret, so a fresh
deploy with a new Terraform-generated password rolls cleanly.

For production, delete this Job and create the admin manually via
`kubectl exec` into a backend pod, prompting for the password
interactively.
...
$ADMIN_PASSWORD = (terraform -chdir=infra output -raw admin_password)
